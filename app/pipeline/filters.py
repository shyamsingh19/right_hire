...
def apply_filters(
    ...
    if jd.min_yoe and jd.min_yoe > 0:
        ...
        if candidate.yoe < yoe_threshold:
            return False, f"Insufficient experience: {candidate.yoe:.1f} yrs < {yoe_threshold:.1f} yrs required"
    ...
    if thresholds.get("location_strict") and jd.location:
        ...
        if jd_loc and cand_loc and jd_loc not in cand_loc and cand_loc not in jd_loc:
            return False, f"Location mismatch: candidate is in '{candidate.location}', JD requires '{jd.location}'"

    # Must-have skills
    if jd.must_haves:
        candidate_skills_lower = {s.lower() for s in candidate.skills}
        missing = [
            skill for skill in jd.must_haves
